...
describe('Download Utils', () => {
    ...
    it('should download repo successfully with valid template', async () => {
      ...
      const mockTemplateContents = [
        { 
          ...
          download_url: 'https://raw.githubusercontent.com/SkipLabs/create-skip-service/main/templates/default/package.json'
        },
        { 
          name: 'README.md', 
          type: 'file', 
          path: 'templates/default/README.md',
          download_url: 'https://raw.githubusercontent.com/SkipLabs/create-skip-service/main/templates/default/README.md'
        }
      ]
  ...
  describe('GitHub API rate limiting', () => {
    it('should handle 403 rate limit error', async () => {
      ...
      await expect(downloadRepo(mockRepo, mockExecutionContext, false))
    ...
    it('should throw rate limit error with correct message', async () => {
      ...
      try {
        ...
        expect((error as CreateSkipServiceError).message).toContain('GitHub API rate limit exceeded')
        expect((error as CreateSkipServiceError).executionContext).toBe(mockExecutionContext)
      }
    })
  })
  ...
  describe('Network failures and error handling', () => {
    ...
    it('should preserve CreateSkipServiceError instances', async () => {
      const originalError = new CreateSkipServiceError('Original error', '/original/path')
      ...
      try {
        ...
        expect((error as CreateSkipServiceError).executionContext).toBe('/original/path')
      }
    })
  })
  ...
  describe('Template validation', () => {
    it('should validate template exists in repository', async () => {
      ...
      await expect(downloadRepo(invalidRepo, mockExecutionContext, false))
    ...
    it('should list available templates when invalid template provided', async () => {
      ...
      try {
        ...
        expect(logger.logError).toHaveBeenCalledWith('  Invalid repository: invalid_template')
        expect(logger.logError).toHaveBeenCalledWith('  Available repositories:')
      ...
      const mockSrcContents = [
        {
          ...
          download_url: 'https://raw.githubusercontent.com/test/repo/main/templates/default/src/index.ts'
        }
      ]
      ...
      const mockTemplateContents = [
        {
          ...
          download_url: 'https://raw.githubusercontent.com/test/repo/main/templates/default/test.ts'
        }
      ]
  ...
  describe('Verbose mode behavior', () => {
    it('should handle verbose mode without errors', async () => {
      const mockTemplateContents = [
        {
          ...
          type: 'file',
          path: 'templates/default/test.ts',
          download_url: 'https://raw.githubusercontent.com/test/repo/main/test.ts'
        }
      ]
      ...
      await expect(downloadRepo(mockRepo, mockExecutionContext, true))
    ...
    it('should handle nested directory structures', async () => {
      ...
      const mockSrcContents = [
        ...
        },
        {
          ...
          type: 'file',
          path: 'templates/default/src/index.ts',
          download_url: 'https://raw.githubusercontent.com/test/repo/main/src/index.ts'
        }
      ]
      ...
      const mockComponentsContents = [
        {
          ...
          download_url: 'https://raw.githubusercontent.com/test/repo/main/src/components/Button.tsx'
        }
      ]
      ...
      mockFetch
        ...
        .mockResolvedValueOnce({
          ok: true,
          status: 200,
          text: () => Promise.resolve('export const Button = () => <button></button>')
        })
        .mockResolvedValueOnce({
          ok: true,
          status: 200,
