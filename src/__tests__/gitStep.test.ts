...
describe('gitStep', () => {
  ...
  describe('Git initialization enabled (withGit: true)', () => {
    it('should initialize git repository successfully with template', async () => {
      ...
      expect(logger.logTitle).toHaveBeenCalledWith(' - Initializing git repository')
      ...
      expect(execa).toHaveBeenCalledWith('git', ['commit', '-m', 
    ...
    it('should initialize git repository successfully with example', async () => {
      const config = { ...baseConfig, withGit: true, template: null, example: mockExample }
      const { logger } = await import('../io.js')
      const { execa } = await import('execa')
  ...
  describe('Git initialization disabled (withGit: false)', () => {
    it('should skip git initialization when withGit is false', async () => {
      ...
      expect(logger.logTitle).toHaveBeenCalledWith(' - Initializing git repository')
      expect(logger.gray).toHaveBeenCalledWith('\t- Skipping git initialization')
  ...
  describe('Commit message generation', () => {
    it('should generate correct commit message for template', async () => {
      const templateConfig = { 
        ...baseConfig, 
        template: { repo: 'CustomOrg/templates', path: 'templates', name: 'custom' },
        example: null 
      }
      const { execa } = await import('execa')
    ...
    it('should generate correct commit message for example', async () => {
      const exampleConfig = { 
        ...baseConfig, 
        template: null,
        example: { repo: 'CustomOrg/examples', path: 'samples', name: 'sample-app' }
      }
      const { execa } = await import('execa')
    ...
    it('should handle template names with special characters', async () => {
      const specialTemplateConfig = { 
        ...baseConfig, 
        template: { repo: 'SkipLabs/create-skip-service', path: 'templates', name: 'template-with_special.chars-123' },
        example: null 
      }
      const { execa } = await import('execa')
    ...
    it('should handle repository names with special characters', async () => {
      const specialRepoConfig = { 
        ...baseConfig, 
        template: { repo: 'Org-Name/repo_with.special-chars', path: 'templates', name: 'default' },
        example: null 
      }
      const { execa } = await import('execa')
  ...
  describe('Error handling', () => {
    ...
    it('should handle different error types', async () => {
      ...
      await expect(gitStep(config)).rejects.toThrow('Invalid git configuration')
    })
  })
