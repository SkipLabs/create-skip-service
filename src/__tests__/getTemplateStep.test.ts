...
describe('getTemplateStep', () => {
  ...
  describe('Template handling', () => {
    ...
    it('should skip when template is undefined', async () => {
      const configWithoutTemplate = { ...mockConfig, template: undefined as any }
      const { logger } = await import('../io.js')
      const { downloadRepo } = await import('../downloadUtils.js')
  ...
  describe('Successful template download', () => {
    ...
    it('should handle different template names', async () => {
      ...
      const configWithCustomTemplate = { ...mockConfig, template: customTemplate }
      const { logger } = await import('../io.js')
      const { downloadRepo } = await import('../downloadUtils.js')
    ...
    it('should handle different repositories', async () => {
      const customTemplate = { ...mockTemplate, repo: 'CustomOrg/custom-templates' }
  ...
  describe('Error handling', () => {
    it('should handle download errors for default template', async () => {
      ...
      await expect(getTemplateStep(mockConfig)).rejects.toThrow('Download failed')
    ...
    it('should show warning for non-default template errors', async () => {
      ...
      await expect(getTemplateStep(configWithCustomTemplate)).rejects.toThrow('Template not found')
    ...
    it('should not show warning for default template errors', async () => {
      ...
      await expect(getTemplateStep(mockConfig)).rejects.toThrow('Network error')
    ...
    it('should handle different error types', async () => {
      ...
      await expect(getTemplateStep(mockConfig)).rejects.toThrow('Invalid response')
      expect(downloadRepo).toHaveBeenCalled()
    })
  })
  ...
  describe('Configuration edge cases', () => {
    ...
    it('should handle template with special characters in name', async () => {
      const specialTemplate = { ...mockTemplate, name: 'template-with_special.chars' }
      const configWithSpecialTemplate = { ...mockConfig, template: specialTemplate }
      const { logger } = await import('../io.js')
      const { downloadRepo } = await import('../downloadUtils.js')
    ...
    it('should handle empty string template name', async () => {
      ...
      const configWithEmptyTemplate = { ...mockConfig, template: emptyTemplate }
      const { logger } = await import('../io.js')
      const { downloadRepo } = await import('../downloadUtils.js')
