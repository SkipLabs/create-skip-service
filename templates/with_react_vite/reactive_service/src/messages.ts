...
class MessageByConversation implements Mapper<
  MessageID,
  Message,
  ConversationID,
  Message
> {
  ...
  ): Iterable<[ConversationID, Message]> {
    const message = values.getUnique();
    return [[message.conversation_id, message]];
  }
}

class FilterByConversation implements Mapper<
  ConversationID,
  Message,
  ConversationID,
  Message
...
class Messages implements Resource<ResourceInputs> {
  ...
  constructor(params: Json) {
    if (typeof params != "number")
      throw new Error("Missing required number parameter 'cid'");
    this.cid = params;
  }

  instantiate(
    inputs: ResourceInputs,
  ): EagerCollection<ConversationID, Message> {
    return inputs.messages
      .map(MessageByConversation)
      .map(FilterByConversation, this.cid);
  }
}
